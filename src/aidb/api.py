"""AIDB REST API — FastAPI server with OpenAPI docs."""
...
log = logging.getLogger("aidb.api")
...
async def lifespan(app: FastAPI):
    """Initialize AIDB + embedder on startup."""
...
async def export_db(include_embeddings: bool = False):
    ...
    from aidb.cli import _export_db
    db, lock = _db()
    with lock:
...
def main():
    """Entry point for the aidb-server console script."""
    host = os.environ.get("AIDB_HOST", "127.0.0.1")
    port = int(os.environ.get("AIDB_PORT", "8321"))
