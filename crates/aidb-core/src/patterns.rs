...
fn mine_co_occurrence(db: &AIDB, config: &PatternConfig) -> Result<Vec<RawPattern>> {
    let conn = db.conn();
...
fn mine_temporal_clusters(db: &AIDB, config: &PatternConfig) -> Result<Vec<RawPattern>> {
    let conn = db.conn();
    let mut stmt = conn.prepare(
...
fn mine_valence_trends(db: &AIDB, _config: &PatternConfig) -> Result<Vec<RawPattern>> {
    let ts = now();
    let conn = db.conn();
...
fn mine_topic_clusters(db: &AIDB, config: &PatternConfig) -> Result<Vec<RawPattern>> {
    let conn = db.conn();
    let mut stmt = conn.prepare(
...
fn mine_entity_hubs(db: &AIDB, config: &PatternConfig) -> Result<Vec<RawPattern>> {
    let conn = db.conn();
    let mut stmt = conn.prepare(
...
fn upsert_pattern(db: &AIDB, raw: &RawPattern, ts: f64) -> Result<bool> {
    let conn = db.conn();
...
pub fn expire_stale_patterns(db: &AIDB, ts: f64, max_age_secs: f64) -> Result<usize> {
    let conn = db.conn();
...
pub fn mine_patterns(db: &AIDB, config: &PatternConfig) -> Result<PatternMiningResult> {
    let ts = now();
    let mut new_count = 0;
