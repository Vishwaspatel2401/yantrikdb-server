...
/// Apply remote ops to a local AIDB instance. Idempotent via INSERT OR IGNORE on op_id.
/// Returns the number of ops actually applied (newly inserted).
pub fn apply_ops(db: &AIDB, ops: &[OplogEntry]) -> Result<SyncStats> {
    let conn = db.conn();
    let mut applied = 0;
    let mut skipped = 0;
...
fn materialize_op(db: &AIDB, op: &OplogEntry) -> Result<()> {
    let conn = db.conn();
